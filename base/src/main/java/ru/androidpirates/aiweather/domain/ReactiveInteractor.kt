package ru.androidpirates.aiweather.domain
...
interface ReactiveInteractor {
    ...
    interface SendInteractor<in Params, Result> : ReactiveInteractor {
     ...
     * It returns an [Flowable] that emits updates for the retrieved object. The returned [Flowable]
     * will never complete, but it can
    ...
    interface RetrieveInteractor<in Params, Object> : ReactiveInteractor {

        fun getBehaviorStream(params: Params): Flowable<Object>
    }

    /**
     * The request interactor is used to request some result once. The returned observable is a single,
     * emits once and then completes or errors.
     *
     * @param <Params> the type of the returned data.
     * @param <Result> required parameters for the request.
    </Result></Params> */
    interface RequestInteractor<in Params, Result> : ReactiveInteractor {
    ...
    interface DeleteInteractor<in Params, Result> : ReactiveInteractor {
     ...
     * this interactor will trigger events in it get interactor counterpart. The returned observable
